...
for idx_sm = 1:n_quantiles
    ...
    estimates(idx_sm, idx_loom, :) = est(~isnan(est));
...
exporter(fh, paths_out, sprintf('across_sm_%d_fits.pdf', idx_loom))


figure(fh)
for idx_quantiles = 1:size(plh,2)
    plh(idx_quantiles).Visible = 'off';


end

exporter(fh, paths_out, sprintf('across_sm_%d_nofits.pdf', idx_loom))


%%

fh = figure('color', 'w', 'Position', [100, 100, 300, 400]);
n_quantiles = 4;
col = cmapper([], n_quantiles);
