...
model_list = {'dddm2', 'dddim2'};
run_list   = {'run14_260325', 'run02_260325'};
label_list = {'with ndt', 'no ndt'};
...
temp_results = importdata(fullfile(results_folder, 'model_results.mat'));
surrogate = importdata(fullfile(results_folder, 'freeze.mat'));
[fh, ax, ax_inset] = fd_conditions('results', temp_results, 'no_y', true, 'vis', true, 'bin_size', 5, 'col', 'gray2', 'censored_inset', true);
...
for idx_model = 1:length(model_list)
    ...
    freezes = importdata(fullfile(results_folder, 'freeze.mat'));
...
exporter(fh, paths, 'cmpr_fits_2.pdf')
