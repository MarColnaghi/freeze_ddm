...
paths = path_generator('folder', 'social_motion/sm_vs_freeze', 'bouts_id', id_code, 'imfirst', false);
...
fh = figure('Position', [100 100 1200 400], 'Color', 'w');
...
for idx_mov_flies = 0:4
    ...
    hold on
    bin_edges = linspace(min(bouts_proc.sm), max(bouts_proc.sm), 100);
    histogram(bouts_proc.sm, 'BinEdges', bin_edges, ...
        'DisplayStyle', 'stairs', 'LineWidth', 1, 'EdgeColor', 'k');
    ...
    for idx_quantiles = 1:3
        ...
        stairs(x, F, 'LineWidth', 2.5, 'Color', col.vars.sm(idx_quantiles + 1, :))
    end
    scatter(0.5, 1, 500,'filled', 'MarkerFaceColor', col.n_mov_flies(idx_mov_flies+1, :));
    text(0.5, 1, num2str(idx_mov_flies), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle', 'FontSize', 20)
    apply_generic(gca, 'xlim', [0 10.5], 'ylim', [-0.02 1.02], 'font_size', 20, 'tick_length', 0.015, 'line_width', 2, 'no_y', true)
end
xlabel('Freeze Duration (s)')
exporter(fh, paths, 'fd_distr_within_clusters.pdf')