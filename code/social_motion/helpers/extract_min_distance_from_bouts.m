function sm_output = extract_min_distance_from_bouts(bouts_le, varargin)
...
distance_cache = importdata(fullfile(paths.cache_path, 'surdistance_cache.mat'));
...
switch opt.Results.type
    ...
    case 'loom'
        ...
        total_looms = 20;
        total_flies = size(distance_cache, 1);
    ...
    case 'onsets'
        ...
        for idx_bouts = 1:total_bouts
            ...
            distance_4 = distance_cache(fly_idx);
            ...
            valid_idx = idx_range > 0 & idx_range <= length(distance_4);

            % Initialize a NaN or zero vector of the required window size
            chunk_data = nan(1, length(idx_range));

            % Fill only the valid portions
            chunk_data(valid_idx) = min(distance_4(idx_range(valid_idx), :), [], 2);

            % Normalize and assign
            switch opt.Results.output_type
                case 'cell'
                    sm_output{idx_bouts} = chunk_data;
    ...
    case 'onlyfreeze'
        ...
        switch opt.Results.output_type
            ...
            case 'cell'
                sm_output = cell(total_bouts, 1);
                for idx_bouts = 1:total_bouts
                    distance_4 = distance_cache(bouts_le.fly(idx_bouts));
                    max_idx = numel(distance_4); % Get the limit for this specific fly
            ...
            case 'mat'
                ...
                for idx_bouts = 1:total_bouts
                    distance_4 = distance_cache(bouts_le.fly(idx_bouts));
                    max_idx = size(distance_4, 1);

                    ons = bouts_le.onsets(idx_bouts);
                    requested_off = bouts_le.ends(idx_bouts) - 1 + delay_after;
                    off = min(requested_off, max_idx);

                    freeze_sig = min(distance_4(ons:off, :), [], 2);
