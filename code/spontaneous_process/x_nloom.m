...
for idx_loom = 1:20
    bouts_loom = bouts_spontaneous(bouts_spontaneous.nloom == idx_loom, :);
    [f, x] = ecdf(bouts_loom.ending_time, 'Censoring', bouts_loom.is_censored); 
    plot(x, f, 'Color', col.vars.nloom(10 + idx_loom,:), 'LineWidth', 1);
end

apply_generic(gca, 'xlim', [0 120], 'ylim', [-0.05 1.05], 'font_size', 20)
