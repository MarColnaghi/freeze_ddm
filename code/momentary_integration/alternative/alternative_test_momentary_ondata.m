...
paths_fit      = path_generator('folder', 'fitting_freezes/le');
...
elseif strcmp(selection, 'random')
    selected = randi(n_bouts, items, 1)';
end
...
for idx_selected_bout = selected %
    ...
    ec.soc_mot_array = sm_signal_us(:, idx_selected_bout);
    ...
    create_vector_for_imagesc = nan(size(t));
    create_vector_for_imagesc(1:length(ec.soc_mot_array)) = ec.soc_mot_array';
    imagesc(t, -0.6, create_vector_for_imagesc, [0 1.5]);  % set XData = time_vector
    ...
    scatter(dur_s, ax.YLim(1) - 0.3, 100, 'filled', '^', 'Clipping', 'off', 'MarkerFaceColor', 'k')

end

exporter(fh, paths_analysis, sprintf('examples_%s_2.pdf'))
