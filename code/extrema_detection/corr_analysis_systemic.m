...
path = '/Users/marcocolnaghi/PhD/freeze_ddm/model_results/extrema_detection/systematic_analysis/run01';
file_list = dir(path);
...
for idx_file = 1:length(folders)
    ...
    for idx_gen_model = {'ac', 'ed'}
        ...
        comparison_sm = importdata(fullfile(stringy, sprintf('comparison_sm_cropped_%s.mat', gen_model)));

        for idx_bout = randi(size(s), 1, 3)
