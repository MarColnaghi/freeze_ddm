...
load('/Users/marcocolnaghi/PhD/freeze_ddm/model_results/momentary_integration/surrogate/dddm2/run16/sims_tv/y.mat')

% Load the motion ts
sim_params.motion_cache_path = fullfile(paths.dataset, 'motion_cache.mat');
load(sim_params.motion_cache_path)

d = 240; percentage_2b_exp = 0.5;
n_selected_comparisons = 150;
...
for idx_bout = 421:450
    ...
    for idx_comparison = 1:height(y)
        ...
        similarity_framebframe = nan(1, length(comparison_sm) - d);

        for idx_frame = 1:length(comparison_sm_cropped) - d
    ...
    for idx_tops = 1:length(best_similarities)
        ...
        %plot(-starting_frame + 1: - starting_frame + length(comparison_sm), comparison_sm, 'Color', col(idx_tops, :))
