function [fh, ax, ax_inset] = fd_conditions_for_cosyne(varargin)
% --- Input Parsing ---
opt = inputParser;
...
col = cmapper();
dx = 1/60; xxi = 0:dx:1200; h = .065; fs = 60;
...
% --- Figure Setup ---
fh = figure('Position', [100 100 1100 700], 'Color', 'w');
...
for idx_sm = 1:3
    for j = 1:4
        i = i + 1;
        if j <= 2, idx_ls = 0; else, idx_ls = 1; end
        ...
        RTD = kreg_single(RTs, RTs, xxi, xbin, h, 0, 500);
                            ...
                            fill_between(xxi, zeros(length(xxi)), RTD{1,2}, [], 'FaceColor', col.empirical.(color)(2*(idx_sm - 1) + idx_fs, :), 'FaceAlpha', 0.25, 'LineStyle', 'none')

                  %  fill_between(xxi, RTD{1,2} + RTD{2,2}, RTD{1,2} - RTD{3,2}, [], 'FaceColor', col.empirical.(color)(2*(idx_sm - 1) + idx_fs, :), 'FaceAlpha', 0.25, 'LineStyle', 'none')
            plot(xxi, RTD{1,2}, 'Color', col.empirical.(color)(2*(idx_sm - 1) + idx_fs, :), 'LineWidth', 2.2)
        ...
        % plot(xxi, ks, 'Color', col.empirical.(color)(2*(idx_sm - 1) + idx_fs, :), 'LineWidth', 1.8);
        ...
        apply_generic(ax(i), 'xlim', [-0.1 10.1], 'ylim', [-0.050 1.550], 'no_y', no_y, 'font_size', 24, 'line_width', 3.5, 'yticks', [0 1], 'xticks', [results.points.truncation 10], 'tick_length', 0.025);
