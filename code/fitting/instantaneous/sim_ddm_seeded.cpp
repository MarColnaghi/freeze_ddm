...
struct FastRNG {
    ...
    FastRNG(uint64_t seed) {
        // Robust initialization using SplitMix64
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    // Expect 4 inputs now: drifts, params, n_trials, SEED
    if (nrhs != 4) mexErrMsgIdAndTxt("sim_ddm_seeded:invalidNumInputs", "Inputs: drifts, params, n_trials, seed");
    ...
    // --- READ SEED ---
    uint64_t seed = (uint64_t)mxGetScalar(prhs[3]);
    ...
    // Initialize RNG with the unique seed passed from MATLAB
    FastGaussian gen(seed); 
